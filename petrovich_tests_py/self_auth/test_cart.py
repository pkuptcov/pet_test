...
class Cart(unittest.TestCase):
    ...
    def test_cart(self):
        ...
        for i in range(60):
            try:
                if re.search(r"^Выберите базу [\s\S]* Время работы: пн-сб: 8:00 – 21:00; вс: 9:00 – 18:00$", driver.find_element_by_css_selector("p.delivery_form_step_header").text): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")

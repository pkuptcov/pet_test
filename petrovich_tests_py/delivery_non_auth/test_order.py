...
class Order(unittest.TestCase):
    ...
    def test_order(self):
        ...
        driver.find_element_by_css_selector("[ng-model=\"orderDeliveryCtrl.order.deliveryAddress\"]").send_keys(u"Россия, Санкт-Петербург, Благодатная улица, 6")
        driver.find_element_by_xpath("(//input[@name='delivery_day'])[3]").click()
        driver.find_element_by_css_selector(u"label:contains('Стандартная доставка')").click()
        Select(driver.find_element_by_xpath("//div[@id='tab_delivery']/ol/li[3]/div/ul/li[2]/div/select")).select_by_visible_text(u"23:30 – 03:30")
        ...
        driver.find_element_by_name("user_name").send_keys(u"Тест")
        driver.find_element_by_css_selector("textarea[ng-model='orderDeliveryCtrl.order.userComment']").clear()
        driver.find_element_by_css_selector("textarea[ng-model='orderDeliveryCtrl.order.userComment']").send_keys(u"тест")
